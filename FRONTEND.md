...
- **Notifications** — The agent can push notifications through the `frontend_command` tool (e.g., "Meeting transcript ready", "EOD summary posted"). These surface as native OS notifications and in an in-app notification center.
...
- **Quick note** — A fast-entry input for capturing notes during meetings or after milestones. Notes land in the brain as human-owned files. The agent picks them up via the watcher.
...
- **Session history** — Conversations persist locally so the user can resume or search previous sessions.
...
The mobile app needs a sync layer since it can't watch the brain folder directly. Options (see QUESTIONS.md):
...
See [QUESTIONS.md](QUESTIONS.md) — frontend-related questions are tracked under **Frontend Surfaces**.
